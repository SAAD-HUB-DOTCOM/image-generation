...
from service.generator import process_job, STYLE_ORDER
from service.imagekit_service import upload_file, get_variants
...
async def create_job(request: CreateJobRequest, session: Session = Depends(get_session)):
    ...
    job = Job(
        ...
        num_thumbnail=request.num_thumbnails,
        headshot_url=request.headshot_url,
    )
    session.add(job)
    session.flush()  # get job.id from database before using it
